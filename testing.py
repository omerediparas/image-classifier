...
def test_model(device, test_loader):
    ...
    model.load_state_dict(torch.load('models/cifar10_model.pt'))  # Load the saved model weights
    ...
    with torch.no_grad():
        ...
        for idx, (images, labels) in enumerate(test_loader):
            images_flat = images.view(images.shape[0],-1)#   # Reshape the images and move to the device
            images_flat = images_flat.to(device)
            labels = labels.to(device)  # DONE Move labels to the device

            # Forward pass: Compute predicted labels
            outputs      =  model(images_flat)# Forward pass
            _, predicted = torch.max(outputs,-1) # DONE Get class with the highest probability
